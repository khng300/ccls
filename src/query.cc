...
void DB::populateVFS(VFS *vfs) const {
  ...
  for (auto &[_, file] : files) {
    if (!file.def)
      continue;
    std::string path = db::toStdString(file.def->path);
    vfs->state[path].loaded = 1;
    vfs->state[path].step = 0;
    vfs->state[path].timestamp = file.def->mtime;
    for (auto &dep : file.def->dependencies)
      step1.insert(std::string(db::toStdString(dep)));
  }
  for (auto &dep : step1)
...
int DB::getFileId(const std::string &path) {
  db::string norm_path(db::toInMemScopedString(lowerPathIfInsensitive(path)));
