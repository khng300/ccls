...
struct QueryFunc : QueryObject<QueryFunc, QueryFuncDef> {
  inline SubdbContainer<EntityID, Usr> deriveds(const DB *db) const;
};

struct QueryType : QueryObject<QueryType, QueryTypeDef> {
  inline SubdbContainer<EntityID, Usr> deriveds(const DB *db) const;
  inline SubdbContainer<EntityID, Usr> instances(const DB *db) const;
};
...
auto QueryObject<QType, QDef>::defs(const DB *db) const -> SubdbContainer<EntityID, Def> {
  return db->entityDefs(*this);
}
template <typename QType, typename QDef>
SubdbContainer<EntityID, DeclRef> QueryObject<QType, QDef>::decls(const DB *db) const {
  return db->entityDecls(*this);
}
template <typename QType, typename QDef>
SubdbContainer<EntityID, Use> QueryObject<QType, QDef>::uses(const DB *db) const {
  return db->entityUses(*this);
}
inline SubdbContainer<EntityID, Usr> QueryFunc::deriveds(const DB *db) const { return (db->entityDeriveds(*this)); }
inline SubdbContainer<EntityID, Usr> QueryType::deriveds(const DB *db) const { return (db->entityDeriveds(*this)); }
inline SubdbContainer<EntityID, Usr> QueryType::instances(const DB *db) const { return (db->entityInstances(*this)); }

template <typename QType, typename QDef> const QDef *QueryObject<QType, QDef>::anyDef(const DB *db) const {
