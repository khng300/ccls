...
public:
  ...
  bool handleDeclOccurrence(const Decl *d, index::SymbolRoleSet roles, ArrayRef<index::SymbolRelation> relations,
    ...
    auto do_def_decl = [&](auto *entity) {
      ...
      if (std::string_view(entity->def.comments).empty() && g_config->index.comments)
    ...
    case Kind::Type:
      ...
      if ((is_def || std::string_view(type->def.detailed_name).empty()) && info->short_name.size()) {
    ...
    case Decl::Record:
      if (auto *tag_d = dyn_cast<TagDecl>(d)) {
        if (std::string_view(type->def.detailed_name).empty() && info->short_name.empty()) {
          StringRef tag;
    ...
    case Decl::CXXMethod:
      ...
      break;
    case Decl::EnumConstant:
      if (is_def && std::string_view(var->def.detailed_name).find_first_of('=') == std::string_view::npos) {
        ...
        var->def.hover = intern(std::string(std::string_view(var->def.detailed_name)) + init);
      }
      break;
    default:
      break;
