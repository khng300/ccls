...
bool expandHelper(MessageHandler *m, DB *db, Out_cclsInheritance *entry,
  ...
  if (def) {
    ...
    if (def->spell) {
      ...
      auto decls = entity.declContainer(*db);
      if (auto dr = decls.begin())
  ...
  if (derived) {
    if (levels > 0) {
      forEach(entity.derivedContainer(*db), [&](auto usr) {
      ...
      entry->numChildren = int(entity.derivedContainer(*db).size());
  } else {
    if (levels > 0) {
