...
void MessageHandler::textDocument_definition(TextDocumentPositionParam &param,
  ...
  if (result.empty()) {
    ...
    if (!range) {
      ...
      forEach(db->allUsrContainer(Kind::Func), [&](const auto id) {
        const auto &func = db->id2Func(id);
        fn({func.usr, Kind::Func});
      });
      forEach(db->allUsrContainer(Kind::Type), [&](const auto id) {
        const auto &type = db->id2Type(id);
        fn({type.usr, Kind::Type});
      });
      forEach(db->allUsrContainer(Kind::Var), [&](const auto id) {
...
void MessageHandler::textDocument_typeDefinition(
  ...
  auto add = [&](const QueryType &type) {
    ...
    if (result.empty()) {
      forEach(type.declContainer(*db), [&](auto dr) {
        if (auto loc = getLocationLink(db, wfiles, dr))
          result.push_back(loc);
      });
    }
  ...
  for (SymbolRef sym : findSymbolsAtLocation(wf, &*file, param.position)) {
    ...
    case Kind::Type: {
      ...
      allOf(type.defContainer(*db), [&](const auto &def) {
        if (def.alias_of) {
