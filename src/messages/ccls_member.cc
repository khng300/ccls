...
void doField(MessageHandler *m, DB *db, Out_cclsMember *entry,
  ...
  if (qualified)
    ...
    entry1.fieldName +=
        def1->detailed_name.view().substr(0, def1->qual_name_offset);
  ...
  if (levels > 0) {
    ...
    while (stack.size()) {
      ...
      if (def->alias_of) {
        ...
        forEach(type->defContainer(*db), [&](const auto &def) {
            ...
            if (seen1.insert(usr).second) {
              ...
              if (auto def1 = db->entityGetAnyDef(func1)) {
                ...
                if (def1->spell) {
                  ...
                  auto decls = func1.declContainer(*db);
                  auto dr = decls.begin();
                  if (dr != decls.end())
            ...
            if (seen1.insert(usr).second) {
              ...
              if (auto def1 = db->entityGetAnyDef(type1)) {
                ...
                if (def1->spell) {
                  ...
                  auto decls = type1.declContainer(*db);
                  auto dr = decls.begin();
                  if (dr != decls.end())
                    if (auto loc = getLsLocation(db, m->wfiles, *dr))
                      entry1.location = *loc;
