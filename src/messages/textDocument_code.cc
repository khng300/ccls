  ...
  for (auto [sym, refcnt] : file->symbol2refcnt) {
    ...
    case Kind::Func: {
      ...
      add("ref", {sym.usr, Kind::Func, "uses"}, sym.range,
          func.useContainer(*db).size(), base_uses.empty());
      if (base_uses.size())
    ...
    case Kind::Var: {
      ...
      add("ref", {sym.usr, Kind::Var, "uses"}, sym.range,
          var.useContainer(*db).size(), def->kind != SymbolKind::Macro);
      break;
    }
    case Kind::File:
    case Kind::Invalid:
  ...
  if (param.command == ccls_xref) {
    ...
    case Kind::Type: {
      ...
      if (cmd.field == "derived") {
        map(getTypeDeclarations(db, type.derivedContainer(*db)));
      } else if (cmd.field == "instances") {
        map(getTypeDeclarations(db, type.instanceContainer(*db)));
      } else if (cmd.field == "uses") {
        map(type.useContainer(*db));
      }
      break;
    }
    case Kind::Var: {
