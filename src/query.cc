...
std::enable_if_t<std::is_same_v<std::decay_t<QueryFileType>, QueryFile>, std::string>
...
EntityID makeEntityID(Kind kind, Usr usr) {
  union {
    struct {
      Usr usr;
      Kind kind;
    } s;
    uint8_t in[sizeof(s)]{0};
  };
  s = {usr, kind};
  union {
    EntityID ret;
  ...
  const uint8_t k[16] = {0x2a, 0xaa, 0x72, 0xa1, 0x36, 0x46, 0x5d, 0xea,
                         0xca, 0xbf, 0x59, 0xb3, 0x44, 0x98, 0xda, 0x19};
  (void)siphash(in, sizeof(in), k, out, 8);
  return ret;
}
...
QueryStore::QueryStore(std::string_view store_path)
  ...
  lmdb::dbi_set_dupsort(txn, entities_defs, [](const MDB_val *a, const MDB_val *b) -> int {
...
template <typename Def> void DB::removeUsrs(int file_id, const std::vector<std::pair<Usr, Def>> &to_remove) {
  constexpr Kind kind =
...
#define REMOVE_ADD(Kind, C, F)                                                                                         \
  ...
  for (auto &p : files) {
    qs_->dbi_files.put(txn_, lmdb::to_sv(p.first), serializeQueryFile(p.second));
  }
#undef REMOVE_ADD
}
...
void DB::populateVFS(VFS *vfs) const {
  ...
  for (const auto &file : filesContainer()) {
    if (!file.def)
      continue;
...
void DB::putFile(int file_id, QueryFile &file) {
  qs_->dbi_files.put(txn_, lmdb::to_sv(file_id), serializeQueryFile(file));
}

template <typename ET, typename Def> void DB::insertEntityDef(ET &&e, Def &&def) {
  auto pdef_buf = cista::serialize(def);
  std::string_view pdef_sv(reinterpret_cast<const char *>(pdef_buf.data()), pdef_buf.size());
  qs_->entities_defs.put(txn_, lmdb::to_sv(e.id), pdef_sv);
}
...
int DB::getFileId(std::string_view path) {
  ...
  if (!lmdb::dbi_put(txn_, qs_->dbi_name2file_id, &key, &val, MDB_NOOVERWRITE)) {
    ...
    qs_->dbi_files.put(txn_, lmdb::to_sv(id), serializeQueryFile(QueryFile{id}));
  }
  return id;
}

template <Kind kind> std::pair<QueryEntityType<kind>, bool> DB::idPutEntity(Usr usr) {
  EntityID id = makeEntityID(kind, usr);
...
int DB::update(std::unordered_map<int, QueryFile> &files, QueryFile::DefUpdate &&u) {
...
void DB::update(std::unordered_map<int, QueryFile> &files, const Lid2file_id &lid2file_id, int file_id,
...
int DB::files_count() const { return qs_->dbi_files.size(txn_); }
uint64_t DB::entities_count(Kind kind) const {
  lmdb::cursor cur = lmdb::cursor::open(txn_, qs_->kind_to_entity_id);
  std::string_view k = lmdb::to_sv(kind);
  if (!cur.get(k, MDB_SET))
    return 0;
  return cur.count();
}
uint64_t DB::funcs_count() const { return entities_count(Kind::Func); }
uint64_t DB::types_count() const { return entities_count(Kind::Type); }
uint64_t DB::vars_count() const { return entities_count(Kind::Var); }

DbContainer<int, QueryFile> DB::filesContainer() const { return DbContainer<int, QueryFile>(txn_, qs_->dbi_files); }

SubdbContainer<EntityID, QueryEntityDef> DB::entityDefContainer(const QueryEntity &entity) const {
  return SubdbContainer<EntityID, QueryEntityDef>(txn_, qs_->entities_defs, entity.id);
}

SubdbContainer<EntityID, DeclRef> DB::entityDeclContainer(const QueryEntity &entity) const {
  return SubdbContainer<EntityID, DeclRef>(txn_, qs_->entities_declarations, entity.id);
}

SubdbContainer<EntityID, Usr> DB::entityDerivedContainer(const QueryEntity &entity) const {
  return SubdbContainer<EntityID, Usr>(txn_, qs_->entities_derived, entity.id);
}

SubdbContainer<EntityID, Usr> DB::entityInstanceContainer(const QueryEntity &entity) const {
  return SubdbContainer<EntityID, Usr>(txn_, qs_->entities_instances, entity.id);
}

SubdbContainer<EntityID, Use> DB::entityUseContainer(const QueryEntity &entity) const {
  return SubdbContainer<EntityID, Use>(txn_, qs_->entities_uses, entity.id);
}

SubdbContainer<Kind, EntityID> DB::allUsrContainer(Kind kind) const {
