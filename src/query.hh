...
template <typename _EntityType, typename QDef> class QueryObject : public QueryEntity {
public:
...
protected:
  ...
  QueryObject(ConstructorKey, const DB &db, const QueryEntity &e) : QueryEntity(e), db_(&db) {}
...
public:
  QueryFunc(ConstructorKey, const DB &db, const QueryEntity &e) : QueryObject(ConstructorKey{}, db, e) {}

  SubdbContainer<EntityID, Usr> deriveds() const;
};
class QueryType : public QueryObject<QueryType, QueryTypeDef> {
public:
  QueryType(ConstructorKey, const DB &db, const QueryEntity &e) : QueryObject(ConstructorKey{}, db, e) {}
...
public:
  QueryVar(ConstructorKey, const DB &db, const QueryEntity &e) : QueryObject(ConstructorKey{}, db, e) {}
};
...
public:
  ...
  QueryFunc id2Func(EntityID id) const { return QueryFunc(QueryFunc::ConstructorKey{}, *this, id2Entity(id)); }
  QueryType id2Type(EntityID id) const { return QueryType(QueryType::ConstructorKey{}, *this, id2Entity(id)); }
  QueryVar id2Var(EntityID id) const { return QueryVar(QueryVar::ConstructorKey{}, *this, id2Entity(id)); }

  QueryFunc getFunc(Usr usr) const { return id2Func(makeEntityID(Kind::Func, usr)); }
  QueryType getType(Usr usr) const { return id2Type(makeEntityID(Kind::Type, usr)); }
  QueryVar getVar(Usr usr) const { return id2Var(makeEntityID(Kind::Var, usr)); }
...
auto QueryObject<_EntityType, QDef>::defs() const -> SubdbContainer<EntityID, Def> {
  return db_->entityDefs(static_cast<const EntityType &>(*this));
}

template <typename _EntityType, typename QDef>
SubdbContainer<EntityID, DeclRef> QueryObject<_EntityType, QDef>::decls() const {
  return db_->entityDecls(static_cast<const EntityType &>(*this));
}

template <typename _EntityType, typename QDef>
SubdbContainer<EntityID, Use> QueryObject<_EntityType, QDef>::uses() const {
  return db_->entityUses(static_cast<const EntityType &>(*this));
}

template <typename _EntityType, typename QDef> const QDef *QueryObject<_EntityType, QDef>::anyDef() const {
  return db_->entityGetAnyDef(static_cast<const EntityType &>(*this));
}
