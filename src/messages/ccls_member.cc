...
bool expand(MessageHandler *m, DB *db, Out_cclsMember *entry, bool qualified,
  ...
  if (levels > 0) {
    ...
    while (stack.size()) {
      ...
      if (def->alias_of) {
        const QueryType::Def *def1 = db->getType(def->alias_of).anyDef();
        Out_cclsMember entry1;
            ...
            if (seen1.insert(usr).second) {
              ...
              if (const QueryFunc::Def *def1 = func1.anyDef()) {
                ...
                if (def1->spell) {
                  ...
                  if (auto loc =
                          getLsLocation(db, m->wfiles, func1.declarations[0]))
                    entry1.location = *loc;
                }
                entry->children.push_back(std::move(entry1));
              }
            ...
            if (seen1.insert(usr).second) {
              ...
              if (const QueryType::Def *def1 = type1.anyDef()) {
                ...
                if (def1->spell) {
                  ...
                  if (auto loc =
                          getLsLocation(db, m->wfiles, type1.declarations[0]))
                    entry1.location = *loc;
                }
                entry->children.push_back(std::move(entry1));
              }
        ...
        for (auto &def : type->def)
          for (auto it : def.vars)
            if (seen1.insert(it.first).second) {
              QueryVar &var = db->getVar(it.first);
              if (!var.def.empty())
...
std::optional<Out_cclsMember> buildInitial(MessageHandler *m, DB *db, Kind kind,
  ...
  case Kind::Type: {
    const auto *def = db->getType(root_usr).anyDef();
    if (!def)
      return {};
  ...
  db->startWrite([&](DB *db) {
    ...
    if (param.id.size()) {
      ...
      if (!(db->hasType(param.usr) &&
            expand(this, db, &*result, param.qualified, param.levels,
                   param.kind)))
        result.reset();
    } else {
