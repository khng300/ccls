...
struct QueryFile {
  ...
  struct DefIndexInclude : IndexIncludeBase<db::scoped_string> {
    ...
    DefIndexInclude(const Alloc &alloc)
        : DefIndexInclude(std::allocator_arg, alloc) {}
    template <typename Alloc>
    DefIndexInclude(std::allocator_arg_t, const Alloc &alloc)
        : IndexIncludeBase{{}, decltype(resolved_path)(alloc)} {}

    template <typename Alloc>
    DefIndexInclude(const DefIndexInclude &rhs, const Alloc &alloc)
        : DefIndexInclude(std::allocator_arg, alloc, rhs) {}
    template <typename Alloc>
    DefIndexInclude(std::allocator_arg_t, const Alloc &alloc,
                    const DefIndexInclude &rhs)
        : IndexIncludeBase{rhs.line,
                           decltype(resolved_path)(rhs.resolved_path, alloc)} {}

    template <typename Alloc>
    DefIndexInclude(DefIndexInclude &&rhs, const Alloc &alloc)
        : DefIndexInclude(std::allocator_arg, alloc, std::move(rhs)) {}
    template <typename Alloc>
    DefIndexInclude(std::allocator_arg_t, const Alloc &alloc,
                    DefIndexInclude &&rhs)
  ...
  struct Def : DefBase<db::scoped_string, DefIndexInclude, db::scoped_vector> {
    template <typename Alloc>
    Def(const Alloc &alloc) : Def(std::allocator_arg, alloc) {}
    template <typename Alloc>
    Def(std::allocator_arg_t, const Alloc &alloc)
    ...
    template <typename Alloc>
    Def(const Def &rhs, const Alloc &alloc)
        : Def(std::allocator_arg, alloc, rhs) {}
    template <typename Alloc>
    Def(std::allocator_arg_t, const Alloc &alloc, const Def &rhs)
    ...
    template <typename Alloc>
    Def(Def &&rhs, const Alloc &alloc)
        : Def(std::allocator_arg, alloc, std::move(rhs)) {}
    template <typename Alloc>
    Def(std::allocator_arg_t, const Alloc &alloc, Def &&rhs)
        : DefBase{
