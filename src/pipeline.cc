...
bool indexer_Parse(SemaManager *completion, WorkingFiles *wfiles,
                   Project *project, VFS *vfs, QueryStoreConnection qs,
...
void indexer_Main(SemaManager *manager, VFS *vfs, QueryStoreConnection qs,
...
void main_OnIndexed(QueryStoreConnection qs, WorkingFiles *wfiles,
                    IndexUpdate *update) {
  if (update->refresh) {
...
void mainLoop() {
  ...
  while (true) {
    ...
    for (int i = 20; i--;) {
      ...
      if (update->files_def_update) {
        auto it = path2backlog.find(
            std::string_view(update->files_def_update->first.path));
