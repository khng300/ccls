...
template <typename _EntityType, typename QDef> struct QueryObject : QueryEntity {
...
public:
  ...
  const QueryFunc &id2Func(EntityID id) const { return id2Entity<QueryFunc>(id); }
  const QueryType &id2Type(EntityID id) const { return id2Entity<QueryType>(id); }
  const QueryVar &id2Var(EntityID id) const { return id2Entity<QueryVar>(id); }

  const QueryFunc &getFunc(Usr usr) const { return id2Func(makeEntityID(Kind::Func, usr)); }
  const QueryType &getType(Usr usr) const { return id2Type(makeEntityID(Kind::Type, usr)); }
  const QueryVar &getVar(Usr usr) const { return id2Var(makeEntityID(Kind::Var, usr)); }
  ...
  uint64_t entities_count(Kind kind) const;
  uint64_t funcs_count() const;
  uint64_t types_count() const;
  uint64_t vars_count() const;

  DbContainer<int, QueryFile> filesContainer() const;
  SubdbContainer<EntityID, QueryEntityDef> entityDefContainer(const QueryEntity &entity) const;
  SubdbContainer<EntityID, DeclRef> entityDeclContainer(const QueryEntity &entity) const;
  SubdbContainer<EntityID, Usr> entityDerivedContainer(const QueryEntity &entity) const;
  SubdbContainer<EntityID, Usr> entityInstanceContainer(const QueryEntity &entity) const;
  SubdbContainer<EntityID, Use> entityUseContainer(const QueryEntity &entity) const;
...
private:
  ...
  template <Kind kind> std::pair<QueryEntityType<kind>, bool> idPutEntity(Usr usr);
...
auto QueryObject<_EntityType, QDef>::defContainer(const DB &db) const -> SubdbContainer<EntityID, Def> {
  return db.entityDefContainer(static_cast<const EntityType &>(*this));
}

template <typename _EntityType, typename QDef>
SubdbContainer<EntityID, DeclRef> QueryObject<_EntityType, QDef>::declContainer(const DB &db) const {
  return db.entityDeclContainer(static_cast<const EntityType &>(*this));
}

template <typename _EntityType, typename QDef>
SubdbContainer<EntityID, Use> QueryObject<_EntityType, QDef>::useContainer(const DB &db) const {
