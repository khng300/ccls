...
QueryStore::QueryStore(std::string_view store_path)
    : env(std::move(env.create().set_max_dbs(48).set_pagesize(4096).open(
          store_path.data(), MDB_WRITEMAP | MDB_MAPASYNC | MDB_NOMETASYNC))) {
...
template <typename CursorType> void DB::removeEntityDef(CursorType &&it) {
  it.del();
}
...
void DB::idRemoveEntity(SubdbCursor<EntityID, QueryEntity> &it) {
  ...
  auto def_cursor = entityDefCursor(entity);
  CCLS_CURSOR_FOREACH(def_cursor, def) {
    (void)def;
    removeEntityDef(def_cursor);
  }
...
DB::entityDefCursor(const QueryEntity &entity) const {
  return SubdbCursor<EntityID, QueryEntityDef>::makeCursor(
      this, txn, qs->entities_defs, entity.id);
}
...
DB::entityDeclCursor(const QueryEntity &entity) const {
  return SubdbCursor<EntityID, DeclRef>::makeCursor(
      this, txn, qs->entities_declarations, entity.id);
}

SubdbCursor<EntityID, Usr>
DB::entityDerivedCursor(const QueryEntity &entity) const {
  return SubdbCursor<EntityID, Usr>::makeCursor(this, txn, qs->entities_derived,
                                                entity.id);
}

SubdbCursor<EntityID, Usr>
DB::entityInstanceCursor(const QueryEntity &entity) const {
  return SubdbCursor<EntityID, Usr>::makeCursor(
      this, txn, qs->entities_instances, entity.id);
}

SubdbCursor<EntityID, Use>
DB::entityUseCursor(const QueryEntity &entity) const {
  return SubdbCursor<EntityID, Use>::makeCursor(this, txn, qs->entities_uses,
                                                entity.id);
}
