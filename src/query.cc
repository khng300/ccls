...
SubdbContainer<EntityID, Usr> QueryFunc::deriveds() const { return (db_->entityDeriveds(*this)); }
SubdbContainer<EntityID, Usr> QueryType::deriveds() const { return (db_->entityDeriveds(*this)); }
SubdbContainer<EntityID, Usr> QueryType::instances() const { return (db_->entityInstances(*this)); }
...
template <typename ET, typename Def> void DB::insertEntityDef(ET &&e, Def &&def) {
  ...
  qs_->entities_defs.put(txn_, lmdb::to_sv(e.id),
                         std::string_view(reinterpret_cast<const char *>(buf.data()), buf.size()));
}
...
std::vector<Use> getUsesForAllDerived(DB *db, const QueryFunc &root) {
  ...
  while (!stack.empty()) {
    QueryFunc func = stack.back();
    stack.pop_back();
    for (const auto &usr : func.deriveds()) {
      if (!seen.count(usr)) {
        QueryFunc func1 = db->getFunc(usr);
        seen.insert(func1.usr);
        stack.push_back(func1);
        for (const auto &use : func1.uses())
          ret.push_back(use);
