...
bool expand(MessageHandler *m, DB *db, Out_cclsInheritance *entry, bool derived,
            bool qualified, int levels);

template <typename Q>
bool expandHelper(MessageHandler *m, DB *db, Out_cclsInheritance *entry,
  ...
  if (def) {
    ...
    if (def->spell) {
      ...
      auto cursor = entity.declCursor(*db);
  ...
  if (derived) {
    if (levels > 0) {
      allOf(entity.derivedCursor(*db), [&](auto usr) {
      ...
      entry->numChildren = int(db->derivedCount(entity));
  } else {
    if (levels > 0) {
      for (auto usr : def->bases.get(db)) {
        if (!seen.insert(usr).second)
          continue;
        Out_cclsInheritance entry1;
        entry1.id = std::to_string(usr);
      ...
      entry->numChildren = int(def->bases.get(db).size());
  }
  return true;
}
...
bool expand(MessageHandler *m, DB *db, Out_cclsInheritance *entry, bool derived,
  ...
  if (entry->kind == Kind::Func)
    return expandHelper(m, db, entry, derived, qualified, levels,
                        db->getFunc(entry->usr));
  else
    return expandHelper(m, db, entry, derived, qualified, levels,
                        db->getType(entry->usr));
}

std::optional<Out_cclsInheritance> buildInitial(MessageHandler *m, DB *db,
...
void inheritance(MessageHandler *m, DB *db, Param &param, ReplyOnce &reply) {
  ...
  if (param.id.size()) {
    ...
    if (!(((param.kind == Kind::Func && db->hasFunc(param.usr)) ||
          ...
          expand(m, db, &*result, param.derived, param.qualified,
                 param.levels)))
      result.reset();
  } else {
    auto [file, wf] =
        m->findOrFail(db, param.textDocument.uri.getPath(), reply);
    if (!wf) {
      return;
    }
    for (SymbolRef sym : findSymbolsAtLocation(wf, &*file, param.position))
      if (sym.kind == Kind::Func || sym.kind == Kind::Type) {
        result = buildInitial(m, db, sym, param.derived, param.qualified,
                              param.levels);
        break;
      }
  }
