...
bool expand(MessageHandler *m, DB *db, Out_cclsMember *entry, bool qualified,
            int levels, Kind memberKind);

// Add a field to |entry| which is a Func/Type.
void doField(MessageHandler *m, DB *db, Out_cclsMember *entry,
             const QueryVar &var, int64_t offset, bool qualified, int levels) {
  auto def1 = db->entityGetAnyDef(var);
  if (!def1)
    return;
  ...
  if (qualified)
    entry1.fieldName += def1->detailed_name.get(db);
  else {
    entry1.fieldName +=
        def1->detailed_name.get(db).substr(0, def1->qual_name_offset);
    entry1.fieldName += def1->name(db, false);
  }
  if (def1->spell) {
    if (std::optional<Location> loc =
            getLsLocation(db, m->wfiles, *def1->spell))
      entry1.location = *loc;
  }
  if (def1->type) {
...
bool expand(MessageHandler *m, DB *db, Out_cclsMember *entry, bool qualified,
  ...
  QueryType type = db->getType(entry->usr);
  auto def = db->entityGetAnyDef(type);
  // builtin types have no declaration and empty |qualified|.
  if (!def)
    return false;
  entry->name = def->name(db, qualified);
  std::unordered_set<Usr> seen;
  if (levels > 0) {
    std::vector<QueryType> stack;
    ...
    while (stack.size()) {
      ...
      if (def->kind != SymbolKind::Namespace)
        for (Usr usr : def->bases.get(db)) {
          auto type1 = db->getType(usr);
          if (db->defCount(type1)) {
            seen.insert(type1.usr);
            stack.push_back(type1);
          }
        }
      if (def->alias_of) {
        QueryType type1 = db->getType(def->alias_of);
        auto def1 = db->entityGetAnyDef(type1);
        Out_cclsMember entry1;
        ...
        if (def1 && def1->spell) {
          ...
          // itself is useful.
          if (std::optional<Location> loc =
                  getLsLocation(db, m->wfiles, *def->spell))
            entry1.location = *loc;
        }
        if (def1 && qualified)
          entry1.fieldName = def1->detailed_name.get(db);
        ...
        allOf(type.defCursor(*db), [&](const auto &def) {
          for (Usr usr : def.funcs.get(db))
            if (seen1.insert(usr).second) {
              const QueryFunc &func1 = db->getFunc(usr);
              if (auto def1 = db->entityGetAnyDef(func1)) {
                Out_cclsMember entry1;
                entry1.fieldName = def1->name(db, false);
                if (def1->spell) {
                  ...
                  auto cursor = func1.declCursor(*db);
                  const auto *dr = cursor ? cursor.first() : nullptr;
                  if (dr != nullptr)
        ...
        llvm::DenseSet<Usr, DenseMapInfoForUsr> seen1;
        allOf(type.defCursor(*db), [&](const auto &def) {
          for (Usr usr : def.types.get(db))
            if (seen1.insert(usr).second) {
              QueryType type1 = db->getType(usr);
              if (auto def1 = db->entityGetAnyDef(type1)) {
                Out_cclsMember entry1;
                entry1.fieldName = def1->name(db, false);
                if (def1->spell) {
                  if (auto loc = getLsLocation(db, m->wfiles, *def1->spell))
                  ...
                  auto cursor = type1.declCursor(*db);
                  const auto *dr = cursor ? cursor.first() : nullptr;
                  if (dr != nullptr)
                    if (auto loc = getLsLocation(db, m->wfiles, *dr))
                      entry1.location = *loc;
    ...
    entry->numChildren = def->alias_of ? 1 : int(def->vars.get(db).size());
  return true;
}

std::optional<Out_cclsMember> buildInitial(MessageHandler *m, DB *db, Kind kind,
  ...
  case Kind::Func: {
    ...
    for (Usr usr : def->vars.get(db)) {
      const QueryVar &var = db->getVar(usr);
      if (db->defCount(var) != 0)
        doField(m, db, &entry, var, -1, qualified, levels - 1);
    }
    return entry;
  }
  case Kind::Type: {
    QueryType type = db->getType(root_usr);
    auto def = db->entityGetAnyDef(type);
    if (!def)
      return {};
...
void MessageHandler::ccls_member(JsonReader &reader, ReplyOnce &reply) {
  ...
  if (param.id.size()) {
    ...
    if (!(db->hasType(param.usr) && expand(this, db, &*result, param.qualified,
    ...
    for (SymbolRef sym : findSymbolsAtLocation(wf, &*file, param.position)) {
      ...
      case Kind::Func:
      case Kind::Type:
        result = buildInitial(this, db, sym.kind, sym.usr, param.qualified,
                              param.levels, param.kind);
        break;
      case Kind::Var: {
        ...
        if (def && def->type)
          result = buildInitial(this, db, Kind::Type, def->type,
                                param.qualified, param.levels, param.kind);
        break;
      }
      default:
        continue;
